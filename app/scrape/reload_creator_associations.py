...
class MarvelRequest():
    def __init__(self):
        self.privateKey = "8d060afde08264584fb23218d71c9e2b2a30de07"
	self.publicKey = "110751b171265917b3734b5c56a689bf"
...
def main():
	...
	fcreators = open('creator_events.txt', 'w')
...
#
	                        ...
	                        if creators_attribute_keys == 'events':
				    #print("Number of events in creators: " + str(creators_attribute['available']))
				    #numEvents = int(creators_attribute['available'])
				    event_ids = [creators['id']]
				    
				    for event_uri in creators_attribute['items'] :
				    	resource_path = event_uri['resourceURI'].split('/')
				    	event_ids.append(int(resource_path[-1]))
				    fcreators.write(str(event_ids) + '\n')
