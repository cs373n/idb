...
#                            #if comic_attribute_keys == 'id':
#                            #    idNum = int(comic_attribute)
                            if comic_attribute_keys == 'series':
                                # print("Comics in comic: " + str(comic_attribute['available']))
                                #numCreators = int(comic_attribute['available'])
                                series_ids = [comic['id']]
                                
				resource_path = comic_attribute['resourceURI'].split('/')
                                series_ids.append(int(resource_path[-1]))
                                fseries.write(str(series_ids) + '\n')


                            elif comic_attribute_keys == 'creators':
                                # print("Comics in comic: " + str(comic_attribute['available']))
                                #numCreators = int(comic_attribute['available'])
                                creator_ids = [comic['id']]
